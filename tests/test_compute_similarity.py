import polars as pl
import numpy as np
from main import (
    ColumnConfig,
    TransformContext,
    extract_player_vector,
    extract_feature_matrix,
    scale_similarity,
    attach_similarity,
)


def test_extract_player_vector():
    transform_context = TransformContext(ColumnConfig(), "719601")

    input_df = pl.DataFrame(
        [
            {
                "UID": "719601",
                "Inf": "",
                "Name": "Robert Lewandowski - Polish",
                "Club": "FC Bayern",
                "Best Pos": "ST (C)",
                "Right Foot": "Very Strong",
                "Left Foot": "Fairly Strong",
                "Mins": "4,064",
                "Hdrs": "278",
                "Hdrs A": "453",
                "Clear": "69.00",
                "Cr A": "24",
                "Cr C": "5",
                "Distance": "535.1km",
                "Drb": "25",
                "FA": "92",
                "Off": "25",
                "Itc": "16",
                "Pas A": "1,594",
                "Ps C": "1,362",
                "CCC": "13",
                "Shots": "261",
                "xG": "40.84",
                "Pens": "15",
                "Tck W": "33",
                "Tck R": "69%",
                "Yel": "4",
                "Red": "0",
                "Fls": "35",
                "Min WD": "N/A",
                "Max WD": "N/A",
                "Min AP": "-",
                "Max AP": "-",
                "Rec": "- -  -  - ",
                "Mins - float": 4064.0,
                "Hdrs A - float": 453.0,
                "Clear - float": 69.0,
                "Cr A - float": 24.0,
                "Drb - float": 25.0,
                "FA - float": 92.0,
                "Itc - float": 16.0,
                "Pas A - float": 1594.0,
                "Ps C - float": 1362.0,
                "Shots - float": 261.0,
                "Pens - float": 15.0,
                "Tck W - float": 33.0,
                "Tck R - float": 0.6900000000000001,
                "Yel - float": 4.0,
                "Red - float": 0.0,
                "Fls - float": 35.0,
                "Tck A": 47.826086956521735,
                "Non Penalty Shots": 246.0,
                "90s": 45.15555555555556,
                "Hdrs A per 90": 10.031988188976378,
                "Clear per 90": 1.528051181102362,
                "Cr A per 90": 0.5314960629921259,
                "Drb per 90": 0.5536417322834646,
                "FA per 90": 2.0374015748031495,
                "Itc per 90": 0.3543307086614173,
                "Pas A per 90": 35.3001968503937,
                "Ps C per 90": 30.162401574803148,
                "Non Penalty Shots per 90": 5.447834645669291,
                "Tck A per 90": 1.059140705237932,
                "Yel per 90": 0.08858267716535433,
                "Red per 90": 0.0,
                "Fls per 90": 0.7750984251968503,
                "Hdrs A per 90 Z": 0.3975874293278999,
                "Clear per 90 Z": -0.7333605233539464,
                "Cr A per 90 Z": -0.7273073514741791,
                "Drb per 90 Z": -0.39194504291950716,
                "Itc per 90 Z": -1.7266260647695595,
                "Pas A per 90 Z": -1.4193264133770644,
                "Non Penalty Shots per 90 Z": 4.0039336464064075,
                "Tck A per 90 Z": -0.8291486717692735,
            },
            {
                "UID": "29179241",
                "Inf": "Inj",
                "Name": "Erling Haaland - Norwegian",
                "Club": "Borussia Dortmund",
                "Best Pos": "ST (C)",
                "Right Foot": "Reasonable",
                "Left Foot": "Very Strong",
                "Mins": "4,025",
                "Hdrs": "353",
                "Hdrs A": "610",
                "Clear": "85.00",
                "Cr A": "67",
                "Cr C": "11",
                "Distance": "591.8km",
                "Drb": "70",
                "FA": "133",
                "Off": "33",
                "Itc": "30",
                "Pas A": "1,704",
                "Ps C": "1,429",
                "CCC": "24",
                "Shots": "289",
                "xG": "37.00",
                "Pens": "9",
                "Tck W": "37",
                "Tck R": "79%",
                "Yel": "1",
                "Red": "0",
                "Fls": "42",
                "Min WD": "£11,399,000 p/a",
                "Max WD": "£15,733,000 p/a",
                "Min AP": "£114M",
                "Max AP": "£134M",
                "Rec": "- -  -  - ",
                "Mins - float": 4025.0,
                "Hdrs A - float": 610.0,
                "Clear - float": 85.0,
                "Cr A - float": 67.0,
                "Drb - float": 70.0,
                "FA - float": 133.0,
                "Itc - float": 30.0,
                "Pas A - float": 1704.0,
                "Ps C - float": 1429.0,
                "Shots - float": 289.0,
                "Pens - float": 9.0,
                "Tck W - float": 37.0,
                "Tck R - float": 0.79,
                "Yel - float": 1.0,
                "Red - float": 0.0,
                "Fls - float": 42.0,
                "Tck A": 46.835443037974684,
                "Non Penalty Shots": 280.0,
                "90s": 44.72222222222222,
                "Hdrs A per 90": 13.639751552795031,
                "Clear per 90": 1.9006211180124224,
                "Cr A per 90": 1.498136645962733,
                "Drb per 90": 1.565217391304348,
                "FA per 90": 2.973913043478261,
                "Itc per 90": 0.6708074534161491,
                "Pas A per 90": 38.10186335403727,
                "Ps C per 90": 31.952795031055903,
                "Non Penalty Shots per 90": 6.260869565217392,
                "Tck A per 90": 1.0472521424640302,
                "Yel per 90": 0.02236024844720497,
                "Red per 90": 0.0,
                "Fls per 90": 0.9391304347826087,
                "Hdrs A per 90 Z": 1.3313508538951497,
                "Clear per 90 Z": -0.6033451624646476,
                "Cr A per 90 Z": -0.3576449639511732,
                "Drb per 90 Z": 0.7752032253627398,
                "Itc per 90 Z": -1.3518956954865335,
                "Pas A per 90 Z": -1.210008876381369,
                "Non Penalty Shots per 90 Z": 4.770446349177681,
                "Tck A per 90 Z": -0.8464824557654527,
            },
            {
                "UID": "62182055",
                "Inf": "nEU",
                "Name": "Dušan Vlahović - Serbian",
                "Club": "Zebre",
                "Best Pos": "ST (C)",
                "Right Foot": "Reasonable",
                "Left Foot": "Very Strong",
                "Mins": "3,950",
                "Hdrs": "292",
                "Hdrs A": "472",
                "Clear": "51.00",
                "Cr A": "30",
                "Cr C": "7",
                "Distance": "542.0km",
                "Drb": "38",
                "FA": "110",
                "Off": "36",
                "Itc": "34",
                "Pas A": "1,682",
                "Ps C": "1,395",
                "CCC": "8",
                "Shots": "195",
                "xG": "26.04",
                "Pens": "10",
                "Tck W": "19",
                "Tck R": "61%",
                "Yel": "1",
                "Red": "0",
                "Fls": "54",
                "Min WD": "£11,748,000 p/a",
                "Max WD": "£15,313,000 p/a",
                "Min AP": "£74M",
                "Max AP": "£92M",
                "Rec": "- -  -  - ",
                "Mins - float": 3950.0,
                "Hdrs A - float": 472.0,
                "Clear - float": 51.0,
                "Cr A - float": 30.0,
                "Drb - float": 38.0,
                "FA - float": 110.0,
                "Itc - float": 34.0,
                "Pas A - float": 1682.0,
                "Ps C - float": 1395.0,
                "Shots - float": 195.0,
                "Pens - float": 10.0,
                "Tck W - float": 19.0,
                "Tck R - float": 0.61,
                "Yel - float": 1.0,
                "Red - float": 0.0,
                "Fls - float": 54.0,
                "Tck A": 31.147540983606557,
                "Non Penalty Shots": 185.0,
                "90s": 43.88888888888889,
                "Hdrs A per 90": 10.754430379746834,
                "Clear per 90": 1.1620253164556962,
                "Cr A per 90": 0.6835443037974683,
                "Drb per 90": 0.8658227848101265,
                "FA per 90": 2.5063291139240502,
                "Itc per 90": 0.7746835443037974,
                "Pas A per 90": 38.32405063291139,
                "Ps C per 90": 31.784810126582276,
                "Non Penalty Shots per 90": 4.215189873417721,
                "Tck A per 90": 0.7096908072214152,
                "Yel per 90": 0.022784810126582275,
                "Red per 90": 0.0,
                "Fls per 90": 1.230379746835443,
                "Hdrs A per 90 Z": 0.5845703398122685,
                "Clear per 90 Z": -0.8610922058443187,
                "Cr A per 90 Z": -0.6691611110773129,
                "Drb per 90 Z": -0.031752929163337135,
                "Itc per 90 Z": -1.228899214566039,
                "Pas A per 90 Z": -1.1934088656715922,
                "Non Penalty Shots per 90 Z": 2.841821351819368,
                "Tck A per 90 Z": -1.3386542499374174,
            },
            {
                "UID": "18007344",
                "Inf": "",
                "Name": "Romelu Lukaku - Belgian",
                "Club": "Chelsea",
                "Best Pos": "ST (C)",
                "Right Foot": "Fairly Strong",
                "Left Foot": "Very Strong",
                "Mins": "3,893",
                "Hdrs": "256",
                "Hdrs A": "447",
                "Clear": "92.00",
                "Cr A": "27",
                "Cr C": "9",
                "Distance": "568.8km",
                "Drb": "25",
                "FA": "99",
                "Off": "15",
                "Itc": "25",
                "Pas A": "1,916",
                "Ps C": "1,658",
                "CCC": "15",
                "Shots": "158",
                "xG": "25.72",
                "Pens": "11",
                "Tck W": "24",
                "Tck R": "83%",
                "Yel": "2",
                "Red": "0",
                "Fls": "24",
                "Min WD": "N/A",
                "Max WD": "N/A",
                "Min AP": "-",
                "Max AP": "-",
                "Rec": "- -  -  - ",
                "Mins - float": 3893.0,
                "Hdrs A - float": 447.0,
                "Clear - float": 92.0,
                "Cr A - float": 27.0,
                "Drb - float": 25.0,
                "FA - float": 99.0,
                "Itc - float": 25.0,
                "Pas A - float": 1916.0,
                "Ps C - float": 1658.0,
                "Shots - float": 158.0,
                "Pens - float": 11.0,
                "Tck W - float": 24.0,
                "Tck R - float": 0.8300000000000001,
                "Yel - float": 2.0,
                "Red - float": 0.0,
                "Fls - float": 24.0,
                "Tck A": 28.915662650602407,
                "Non Penalty Shots": 147.0,
                "90s": 43.25555555555556,
                "Hdrs A per 90": 10.33393269971744,
                "Clear per 90": 2.1268944258926274,
                "Cr A per 90": 0.6241972771641408,
                "Drb per 90": 0.5779604418186488,
                "FA per 90": 2.2887233496018493,
                "Itc per 90": 0.5779604418186488,
                "Pas A per 90": 44.294888260981246,
                "Ps C per 90": 38.33033650141279,
                "Non Penalty Shots per 90": 3.398407397893655,
                "Tck A per 90": 0.6684843664408467,
                "Yel per 90": 0.0462368353454919,
                "Red per 90": 0.0,
                "Fls per 90": 0.5548420241459029,
                "Hdrs A per 90 Z": 0.47573688425235805,
                "Clear per 90 Z": -0.5243827909226783,
                "Cr A per 90 Z": -0.6918565823250108,
                "Drb per 90 Z": -0.3638863016204905,
                "Itc per 90 Z": -1.4618329832001087,
                "Pas A per 90 Z": -0.7473168613293786,
                "Non Penalty Shots per 90 Z": 2.071775529878353,
                "Tck A per 90 Z": -1.3987341399397393,
            },
            {
                "UID": "67220143",
                "Inf": "",
                "Name": "Mikel Oyarzábal - Spanish",
                "Club": "Real San Sebastián",
                "Best Pos": "AM (R)",
                "Right Foot": "Reasonable",
                "Left Foot": "Very Strong",
                "Mins": "3,966",
                "Hdrs": "249",
                "Hdrs A": "495",
                "Clear": "52.00",
                "Cr A": "72",
                "Cr C": "24",
                "Distance": "564.0km",
                "Drb": "53",
                "FA": "89",
                "Off": "25",
                "Itc": "39",
                "Pas A": "1,939",
                "Ps C": "1,633",
                "CCC": "12",
                "Shots": "224",
                "xG": "24.19",
                "Pens": "2",
                "Tck W": "42",
                "Tck R": "81%",
                "Yel": "0",
                "Red": "1",
                "Fls": "42",
                "Min WD": "£5,327,000 p/a",
                "Max WD": "£7,172,000 p/a",
                "Min AP": "£64M",
                "Max AP": "£64M",
                "Rec": "- -  -  - ",
                "Mins - float": 3966.0,
                "Hdrs A - float": 495.0,
                "Clear - float": 52.0,
                "Cr A - float": 72.0,
                "Drb - float": 53.0,
                "FA - float": 89.0,
                "Itc - float": 39.0,
                "Pas A - float": 1939.0,
                "Ps C - float": 1633.0,
                "Shots - float": 224.0,
                "Pens - float": 2.0,
                "Tck W - float": 42.0,
                "Tck R - float": 0.81,
                "Yel - float": 0.0,
                "Red - float": 1.0,
                "Fls - float": 42.0,
                "Tck A": 51.85185185185185,
                "Non Penalty Shots": 222.0,
                "90s": 44.06666666666667,
                "Hdrs A per 90": 11.232980332829046,
                "Clear per 90": 1.1800302571860817,
                "Cr A per 90": 1.6338880484114977,
                "Drb per 90": 1.2027231467473525,
                "FA per 90": 2.019667170953101,
                "Itc per 90": 0.8850226928895613,
                "Pas A per 90": 44.00151285930408,
                "Ps C per 90": 37.05748865355522,
                "Non Penalty Shots per 90": 5.0378214826021175,
                "Tck A per 90": 1.1766683476214488,
                "Yel per 90": 0.0,
                "Red per 90": 0.0226928895612708,
                "Fls per 90": 0.9531013615733736,
                "Hdrs A per 90 Z": 0.7084289186475415,
                "Clear per 90 Z": -0.8548090395581012,
                "Cr A per 90 Z": -0.3057309553497645,
                "Drb per 90 Z": 0.3569601349680111,
                "Itc per 90 Z": -1.0982500260199355,
                "Pas A per 90 Z": -0.7692354645658186,
                "Non Penalty Shots per 90 Z": 3.617381617412252,
                "Tck A per 90 Z": -0.6577908040942849,
            },
        ]
    )

    output = extract_player_vector(input_df, transform_context)

    expected = np.array(
        [
            [
                0.39758743,
                -0.73336052,
                -0.72730735,
                -0.39194504,
                -1.72662606,
                -1.41932641,
                4.00393365,
                -0.82914867,
            ]
        ],
        dtype=float,
    )

    np.testing.assert_allclose(
        output,
        expected,
    )


def test_extract_feature_matrix():
    transform_context = TransformContext(ColumnConfig(), "719601")

    input_df = pl.DataFrame(
        [
            {
                "UID": "719601",
                "Inf": "",
                "Name": "Robert Lewandowski - Polish",
                "Club": "FC Bayern",
                "Best Pos": "ST (C)",
                "Right Foot": "Very Strong",
                "Left Foot": "Fairly Strong",
                "Mins": "4,064",
                "Hdrs": "278",
                "Hdrs A": "453",
                "Clear": "69.00",
                "Cr A": "24",
                "Cr C": "5",
                "Distance": "535.1km",
                "Drb": "25",
                "FA": "92",
                "Off": "25",
                "Itc": "16",
                "Pas A": "1,594",
                "Ps C": "1,362",
                "CCC": "13",
                "Shots": "261",
                "xG": "40.84",
                "Pens": "15",
                "Tck W": "33",
                "Tck R": "69%",
                "Yel": "4",
                "Red": "0",
                "Fls": "35",
                "Min WD": "N/A",
                "Max WD": "N/A",
                "Min AP": "-",
                "Max AP": "-",
                "Rec": "- -  -  - ",
                "Mins - float": 4064.0,
                "Hdrs A - float": 453.0,
                "Clear - float": 69.0,
                "Cr A - float": 24.0,
                "Drb - float": 25.0,
                "FA - float": 92.0,
                "Itc - float": 16.0,
                "Pas A - float": 1594.0,
                "Ps C - float": 1362.0,
                "Shots - float": 261.0,
                "Pens - float": 15.0,
                "Tck W - float": 33.0,
                "Tck R - float": 0.6900000000000001,
                "Yel - float": 4.0,
                "Red - float": 0.0,
                "Fls - float": 35.0,
                "Tck A": 47.826086956521735,
                "Non Penalty Shots": 246.0,
                "90s": 45.15555555555556,
                "Hdrs A per 90": 10.031988188976378,
                "Clear per 90": 1.528051181102362,
                "Cr A per 90": 0.5314960629921259,
                "Drb per 90": 0.5536417322834646,
                "FA per 90": 2.0374015748031495,
                "Itc per 90": 0.3543307086614173,
                "Pas A per 90": 35.3001968503937,
                "Ps C per 90": 30.162401574803148,
                "Non Penalty Shots per 90": 5.447834645669291,
                "Tck A per 90": 1.059140705237932,
                "Yel per 90": 0.08858267716535433,
                "Red per 90": 0.0,
                "Fls per 90": 0.7750984251968503,
                "Hdrs A per 90 Z": 0.3975874293278999,
                "Clear per 90 Z": -0.7333605233539464,
                "Cr A per 90 Z": -0.7273073514741791,
                "Drb per 90 Z": -0.39194504291950716,
                "Itc per 90 Z": -1.7266260647695595,
                "Pas A per 90 Z": -1.4193264133770644,
                "Non Penalty Shots per 90 Z": 4.0039336464064075,
                "Tck A per 90 Z": -0.8291486717692735,
            },
            {
                "UID": "29179241",
                "Inf": "Inj",
                "Name": "Erling Haaland - Norwegian",
                "Club": "Borussia Dortmund",
                "Best Pos": "ST (C)",
                "Right Foot": "Reasonable",
                "Left Foot": "Very Strong",
                "Mins": "4,025",
                "Hdrs": "353",
                "Hdrs A": "610",
                "Clear": "85.00",
                "Cr A": "67",
                "Cr C": "11",
                "Distance": "591.8km",
                "Drb": "70",
                "FA": "133",
                "Off": "33",
                "Itc": "30",
                "Pas A": "1,704",
                "Ps C": "1,429",
                "CCC": "24",
                "Shots": "289",
                "xG": "37.00",
                "Pens": "9",
                "Tck W": "37",
                "Tck R": "79%",
                "Yel": "1",
                "Red": "0",
                "Fls": "42",
                "Min WD": "£11,399,000 p/a",
                "Max WD": "£15,733,000 p/a",
                "Min AP": "£114M",
                "Max AP": "£134M",
                "Rec": "- -  -  - ",
                "Mins - float": 4025.0,
                "Hdrs A - float": 610.0,
                "Clear - float": 85.0,
                "Cr A - float": 67.0,
                "Drb - float": 70.0,
                "FA - float": 133.0,
                "Itc - float": 30.0,
                "Pas A - float": 1704.0,
                "Ps C - float": 1429.0,
                "Shots - float": 289.0,
                "Pens - float": 9.0,
                "Tck W - float": 37.0,
                "Tck R - float": 0.79,
                "Yel - float": 1.0,
                "Red - float": 0.0,
                "Fls - float": 42.0,
                "Tck A": 46.835443037974684,
                "Non Penalty Shots": 280.0,
                "90s": 44.72222222222222,
                "Hdrs A per 90": 13.639751552795031,
                "Clear per 90": 1.9006211180124224,
                "Cr A per 90": 1.498136645962733,
                "Drb per 90": 1.565217391304348,
                "FA per 90": 2.973913043478261,
                "Itc per 90": 0.6708074534161491,
                "Pas A per 90": 38.10186335403727,
                "Ps C per 90": 31.952795031055903,
                "Non Penalty Shots per 90": 6.260869565217392,
                "Tck A per 90": 1.0472521424640302,
                "Yel per 90": 0.02236024844720497,
                "Red per 90": 0.0,
                "Fls per 90": 0.9391304347826087,
                "Hdrs A per 90 Z": 1.3313508538951497,
                "Clear per 90 Z": -0.6033451624646476,
                "Cr A per 90 Z": -0.3576449639511732,
                "Drb per 90 Z": 0.7752032253627398,
                "Itc per 90 Z": -1.3518956954865335,
                "Pas A per 90 Z": -1.210008876381369,
                "Non Penalty Shots per 90 Z": 4.770446349177681,
                "Tck A per 90 Z": -0.8464824557654527,
            },
            {
                "UID": "62182055",
                "Inf": "nEU",
                "Name": "Dušan Vlahović - Serbian",
                "Club": "Zebre",
                "Best Pos": "ST (C)",
                "Right Foot": "Reasonable",
                "Left Foot": "Very Strong",
                "Mins": "3,950",
                "Hdrs": "292",
                "Hdrs A": "472",
                "Clear": "51.00",
                "Cr A": "30",
                "Cr C": "7",
                "Distance": "542.0km",
                "Drb": "38",
                "FA": "110",
                "Off": "36",
                "Itc": "34",
                "Pas A": "1,682",
                "Ps C": "1,395",
                "CCC": "8",
                "Shots": "195",
                "xG": "26.04",
                "Pens": "10",
                "Tck W": "19",
                "Tck R": "61%",
                "Yel": "1",
                "Red": "0",
                "Fls": "54",
                "Min WD": "£11,748,000 p/a",
                "Max WD": "£15,313,000 p/a",
                "Min AP": "£74M",
                "Max AP": "£92M",
                "Rec": "- -  -  - ",
                "Mins - float": 3950.0,
                "Hdrs A - float": 472.0,
                "Clear - float": 51.0,
                "Cr A - float": 30.0,
                "Drb - float": 38.0,
                "FA - float": 110.0,
                "Itc - float": 34.0,
                "Pas A - float": 1682.0,
                "Ps C - float": 1395.0,
                "Shots - float": 195.0,
                "Pens - float": 10.0,
                "Tck W - float": 19.0,
                "Tck R - float": 0.61,
                "Yel - float": 1.0,
                "Red - float": 0.0,
                "Fls - float": 54.0,
                "Tck A": 31.147540983606557,
                "Non Penalty Shots": 185.0,
                "90s": 43.88888888888889,
                "Hdrs A per 90": 10.754430379746834,
                "Clear per 90": 1.1620253164556962,
                "Cr A per 90": 0.6835443037974683,
                "Drb per 90": 0.8658227848101265,
                "FA per 90": 2.5063291139240502,
                "Itc per 90": 0.7746835443037974,
                "Pas A per 90": 38.32405063291139,
                "Ps C per 90": 31.784810126582276,
                "Non Penalty Shots per 90": 4.215189873417721,
                "Tck A per 90": 0.7096908072214152,
                "Yel per 90": 0.022784810126582275,
                "Red per 90": 0.0,
                "Fls per 90": 1.230379746835443,
                "Hdrs A per 90 Z": 0.5845703398122685,
                "Clear per 90 Z": -0.8610922058443187,
                "Cr A per 90 Z": -0.6691611110773129,
                "Drb per 90 Z": -0.031752929163337135,
                "Itc per 90 Z": -1.228899214566039,
                "Pas A per 90 Z": -1.1934088656715922,
                "Non Penalty Shots per 90 Z": 2.841821351819368,
                "Tck A per 90 Z": -1.3386542499374174,
            },
            {
                "UID": "18007344",
                "Inf": "",
                "Name": "Romelu Lukaku - Belgian",
                "Club": "Chelsea",
                "Best Pos": "ST (C)",
                "Right Foot": "Fairly Strong",
                "Left Foot": "Very Strong",
                "Mins": "3,893",
                "Hdrs": "256",
                "Hdrs A": "447",
                "Clear": "92.00",
                "Cr A": "27",
                "Cr C": "9",
                "Distance": "568.8km",
                "Drb": "25",
                "FA": "99",
                "Off": "15",
                "Itc": "25",
                "Pas A": "1,916",
                "Ps C": "1,658",
                "CCC": "15",
                "Shots": "158",
                "xG": "25.72",
                "Pens": "11",
                "Tck W": "24",
                "Tck R": "83%",
                "Yel": "2",
                "Red": "0",
                "Fls": "24",
                "Min WD": "N/A",
                "Max WD": "N/A",
                "Min AP": "-",
                "Max AP": "-",
                "Rec": "- -  -  - ",
                "Mins - float": 3893.0,
                "Hdrs A - float": 447.0,
                "Clear - float": 92.0,
                "Cr A - float": 27.0,
                "Drb - float": 25.0,
                "FA - float": 99.0,
                "Itc - float": 25.0,
                "Pas A - float": 1916.0,
                "Ps C - float": 1658.0,
                "Shots - float": 158.0,
                "Pens - float": 11.0,
                "Tck W - float": 24.0,
                "Tck R - float": 0.8300000000000001,
                "Yel - float": 2.0,
                "Red - float": 0.0,
                "Fls - float": 24.0,
                "Tck A": 28.915662650602407,
                "Non Penalty Shots": 147.0,
                "90s": 43.25555555555556,
                "Hdrs A per 90": 10.33393269971744,
                "Clear per 90": 2.1268944258926274,
                "Cr A per 90": 0.6241972771641408,
                "Drb per 90": 0.5779604418186488,
                "FA per 90": 2.2887233496018493,
                "Itc per 90": 0.5779604418186488,
                "Pas A per 90": 44.294888260981246,
                "Ps C per 90": 38.33033650141279,
                "Non Penalty Shots per 90": 3.398407397893655,
                "Tck A per 90": 0.6684843664408467,
                "Yel per 90": 0.0462368353454919,
                "Red per 90": 0.0,
                "Fls per 90": 0.5548420241459029,
                "Hdrs A per 90 Z": 0.47573688425235805,
                "Clear per 90 Z": -0.5243827909226783,
                "Cr A per 90 Z": -0.6918565823250108,
                "Drb per 90 Z": -0.3638863016204905,
                "Itc per 90 Z": -1.4618329832001087,
                "Pas A per 90 Z": -0.7473168613293786,
                "Non Penalty Shots per 90 Z": 2.071775529878353,
                "Tck A per 90 Z": -1.3987341399397393,
            },
            {
                "UID": "67220143",
                "Inf": "",
                "Name": "Mikel Oyarzábal - Spanish",
                "Club": "Real San Sebastián",
                "Best Pos": "AM (R)",
                "Right Foot": "Reasonable",
                "Left Foot": "Very Strong",
                "Mins": "3,966",
                "Hdrs": "249",
                "Hdrs A": "495",
                "Clear": "52.00",
                "Cr A": "72",
                "Cr C": "24",
                "Distance": "564.0km",
                "Drb": "53",
                "FA": "89",
                "Off": "25",
                "Itc": "39",
                "Pas A": "1,939",
                "Ps C": "1,633",
                "CCC": "12",
                "Shots": "224",
                "xG": "24.19",
                "Pens": "2",
                "Tck W": "42",
                "Tck R": "81%",
                "Yel": "0",
                "Red": "1",
                "Fls": "42",
                "Min WD": "£5,327,000 p/a",
                "Max WD": "£7,172,000 p/a",
                "Min AP": "£64M",
                "Max AP": "£64M",
                "Rec": "- -  -  - ",
                "Mins - float": 3966.0,
                "Hdrs A - float": 495.0,
                "Clear - float": 52.0,
                "Cr A - float": 72.0,
                "Drb - float": 53.0,
                "FA - float": 89.0,
                "Itc - float": 39.0,
                "Pas A - float": 1939.0,
                "Ps C - float": 1633.0,
                "Shots - float": 224.0,
                "Pens - float": 2.0,
                "Tck W - float": 42.0,
                "Tck R - float": 0.81,
                "Yel - float": 0.0,
                "Red - float": 1.0,
                "Fls - float": 42.0,
                "Tck A": 51.85185185185185,
                "Non Penalty Shots": 222.0,
                "90s": 44.06666666666667,
                "Hdrs A per 90": 11.232980332829046,
                "Clear per 90": 1.1800302571860817,
                "Cr A per 90": 1.6338880484114977,
                "Drb per 90": 1.2027231467473525,
                "FA per 90": 2.019667170953101,
                "Itc per 90": 0.8850226928895613,
                "Pas A per 90": 44.00151285930408,
                "Ps C per 90": 37.05748865355522,
                "Non Penalty Shots per 90": 5.0378214826021175,
                "Tck A per 90": 1.1766683476214488,
                "Yel per 90": 0.0,
                "Red per 90": 0.0226928895612708,
                "Fls per 90": 0.9531013615733736,
                "Hdrs A per 90 Z": 0.7084289186475415,
                "Clear per 90 Z": -0.8548090395581012,
                "Cr A per 90 Z": -0.3057309553497645,
                "Drb per 90 Z": 0.3569601349680111,
                "Itc per 90 Z": -1.0982500260199355,
                "Pas A per 90 Z": -0.7692354645658186,
                "Non Penalty Shots per 90 Z": 3.617381617412252,
                "Tck A per 90 Z": -0.6577908040942849,
            },
        ]
    )

    output = extract_feature_matrix(input_df, transform_context)

    expected = np.array(
        [
            [
                0.39758743,
                -0.73336052,
                -0.72730735,
                -0.39194504,
                -1.72662606,
                -1.41932641,
                4.00393365,
                -0.82914867,
            ],
            [
                1.33135085,
                -0.60334516,
                -0.35764496,
                0.77520323,
                -1.3518957,
                -1.21000888,
                4.77044635,
                -0.84648246,
            ],
            [
                0.58457034,
                -0.86109221,
                -0.66916111,
                -0.03175293,
                -1.22889921,
                -1.19340887,
                2.84182135,
                -1.33865425,
            ],
            [
                0.47573688,
                -0.52438279,
                -0.69185658,
                -0.3638863,
                -1.46183298,
                -0.74731686,
                2.07177553,
                -1.39873414,
            ],
            [
                0.70842892,
                -0.85480904,
                -0.30573096,
                0.35696013,
                -1.09825003,
                -0.76923546,
                3.61738162,
                -0.6577908,
            ],
        ],
        dtype=float,
    )

    np.testing.assert_allclose(
        output,
        expected,
    )


def test_scale_similarity():
    input = np.array([-0.17972866, -0.26922463, -0.16451044, -0.02216566, -0.18559143])

    output = scale_similarity(input)

    expected = np.array([41.01356689, 36.53876832, 41.7744782, 48.891717, 40.72042839])

    np.testing.assert_allclose(
        output,
        expected,
    )


def test_attach_similarity():
    df = pl.DataFrame(
        [
            {
                "UID": "719601",
                "Hdrs A per 90 Z": 0.3975874293278999,
                "Clear per 90 Z": -0.7333605233539464,
                "Cr A per 90 Z": -0.7273073514741791,
                "Drb per 90 Z": -0.39194504291950716,
                "Itc per 90 Z": -1.7266260647695595,
                "Pas A per 90 Z": -1.4193264133770644,
                "Non Penalty Shots per 90 Z": 4.0039336464064075,
                "Tck A per 90 Z": -0.8291486717692735,
            },
            {
                "UID": "29179241",
                "Hdrs A per 90 Z": 1.3313508538951497,
                "Clear per 90 Z": -0.6033451624646476,
                "Cr A per 90 Z": -0.3576449639511732,
                "Drb per 90 Z": 0.7752032253627398,
                "Itc per 90 Z": -1.3518956954865335,
                "Pas A per 90 Z": -1.210008876381369,
                "Non Penalty Shots per 90 Z": 4.770446349177681,
                "Tck A per 90 Z": -0.8464824557654527,
            },
            {
                "UID": "62182055",
                "Hdrs A per 90 Z": 0.5845703398122685,
                "Clear per 90 Z": -0.8610922058443187,
                "Cr A per 90 Z": -0.6691611110773129,
                "Drb per 90 Z": -0.031752929163337135,
                "Itc per 90 Z": -1.228899214566039,
                "Pas A per 90 Z": -1.1934088656715922,
                "Non Penalty Shots per 90 Z": 2.841821351819368,
                "Tck A per 90 Z": -1.3386542499374174,
            },
            {
                "UID": "18007344",
                "Hdrs A per 90 Z": 0.47573688425235805,
                "Clear per 90 Z": -0.5243827909226783,
                "Cr A per 90 Z": -0.6918565823250108,
                "Drb per 90 Z": -0.3638863016204905,
                "Itc per 90 Z": -1.4618329832001087,
                "Pas A per 90 Z": -0.7473168613293786,
                "Non Penalty Shots per 90 Z": 2.071775529878353,
                "Tck A per 90 Z": -1.3987341399397393,
            },
            {
                "UID": "67220143",
                "Hdrs A per 90 Z": 0.7084289186475415,
                "Clear per 90 Z": -0.8548090395581012,
                "Cr A per 90 Z": -0.3057309553497645,
                "Drb per 90 Z": 0.3569601349680111,
                "Itc per 90 Z": -1.0982500260199355,
                "Pas A per 90 Z": -0.7692354645658186,
                "Non Penalty Shots per 90 Z": 3.617381617412252,
                "Tck A per 90 Z": -0.6577908040942849,
            },
        ]
    )

    similarity_Scores = np.array(
        [41.01356689, 36.53876832, 41.7744782, 48.891717, 40.72042839]
    )

    output_df = attach_similarity(df, similarity_Scores)

    expected_df = pl.DataFrame(
        [
            {
                "UID": "719601",
                "Hdrs A per 90 Z": 0.3975874293278999,
                "Clear per 90 Z": -0.7333605233539464,
                "Cr A per 90 Z": -0.7273073514741791,
                "Drb per 90 Z": -0.39194504291950716,
                "Itc per 90 Z": -1.7266260647695595,
                "Pas A per 90 Z": -1.4193264133770644,
                "Non Penalty Shots per 90 Z": 4.0039336464064075,
                "Tck A per 90 Z": -0.8291486717692735,
                "Similarity": 41.01356689,
            },
            {
                "UID": "29179241",
                "Hdrs A per 90 Z": 1.3313508538951497,
                "Clear per 90 Z": -0.6033451624646476,
                "Cr A per 90 Z": -0.3576449639511732,
                "Drb per 90 Z": 0.7752032253627398,
                "Itc per 90 Z": -1.3518956954865335,
                "Pas A per 90 Z": -1.210008876381369,
                "Non Penalty Shots per 90 Z": 4.770446349177681,
                "Tck A per 90 Z": -0.8464824557654527,
                "Similarity": 36.53876832,
            },
            {
                "UID": "62182055",
                "Hdrs A per 90 Z": 0.5845703398122685,
                "Clear per 90 Z": -0.8610922058443187,
                "Cr A per 90 Z": -0.6691611110773129,
                "Drb per 90 Z": -0.031752929163337135,
                "Itc per 90 Z": -1.228899214566039,
                "Pas A per 90 Z": -1.1934088656715922,
                "Non Penalty Shots per 90 Z": 2.841821351819368,
                "Tck A per 90 Z": -1.3386542499374174,
                "Similarity": 41.7744782,
            },
            {
                "UID": "18007344",
                "Hdrs A per 90 Z": 0.47573688425235805,
                "Clear per 90 Z": -0.5243827909226783,
                "Cr A per 90 Z": -0.6918565823250108,
                "Drb per 90 Z": -0.3638863016204905,
                "Itc per 90 Z": -1.4618329832001087,
                "Pas A per 90 Z": -0.7473168613293786,
                "Non Penalty Shots per 90 Z": 2.071775529878353,
                "Tck A per 90 Z": -1.3987341399397393,
                "Similarity": 48.891717,
            },
            {
                "UID": "67220143",
                "Hdrs A per 90 Z": 0.7084289186475415,
                "Clear per 90 Z": -0.8548090395581012,
                "Cr A per 90 Z": -0.3057309553497645,
                "Drb per 90 Z": 0.3569601349680111,
                "Itc per 90 Z": -1.0982500260199355,
                "Pas A per 90 Z": -0.7692354645658186,
                "Non Penalty Shots per 90 Z": 3.617381617412252,
                "Tck A per 90 Z": -0.6577908040942849,
                "Similarity": 40.72042839,
            },
        ]
    )
